...
// uniform FrameState {
// uniform float[6] u_coordinate_to_pixel;
// uniform float[2] u_viewport_size;
...
//   float u_adfFromGeoX[10];
//   float u_adfFromGeoY[10];
...
// uniform mat2x3 u_coordinate_to_pixel;
uniform float[6] u_coordinate_to_pixel;
uniform float[2] u_viewport_size;
...
vec2 GDALGCPTransform(float x2_mean, float y2_mean, float[10] adfFromGeoX, float[10] adfFromGeoY, int nOrder, vec2 point) {
  vec2 transformedPoint = CRS_georef(point.x - x2_mean, point.y - y2_mean, adfFromGeoX, adfFromGeoY, nOrder);

  return transformedPoint;
}

// vec2 apply(mat2x3 transform, vec4 coordinate) {
vec2 apply(float[6] transform, vec4 coordinate) {
  float x = coordinate[0];
  float y = coordinate[1];

  // return vec2(
  //   transform[0][0] * x + transform[0][2] * y + transform[1][1],
  //   transform[0][1] * x + transform[1][0] * y + transform[1][2]
  // );
...
void main() {
  vec2 pixel = apply(u_coordinate_to_pixel, a_position);

  float x = pixel[0] / u_viewport_size[0] * 2.0 - 1.0;
  float y = -(pixel[1] / u_viewport_size[1] * 2.0 - 1.0);

  gl_Position = vec4(x, y, 0.0, 1.0);
  v_vertex_color = vec4(a_color[0], a_color[1], a_color[2], 0.4);
}
